...
import { useAuth } from "@/contexts/auth-context" // fixed import path to use absolute path
...
export default function LoginScreen() {
  ...
  const { user, login, verifyOTP, loading } = useAuth() // added login and loading to destructuring
  ...
  const handleSendOTP = async () => {
    ...
    try {
      await login(`+91${phone}`, "pet_owner")
      ...
      <StyledView className="bg-white p-4 rounded-lg shadow-md">
        {step === "phone" ? (
          <>
            ...
            >
              {isLoading || loading ? (
                ...
                <StyledText className="text-white font-semibold">Send OTP</StyledText>
              )}
            </StyledTouchable>
          </>
        ) : (
          <>
            ...
            <StyledTouchable
              className="bg-zubo-primary p-3 rounded-md items-center mb-2"
            ...
            >
              {isLoading || loading ? (
                ...
                <StyledText className="text-white font-semibold">Verify OTP</StyledText>
              )}
            </StyledTouchable>
