...
export default function PetDetailsPage({ params }: { params: { id: string } }) {
  ...
  // Ensure weight is a number if possible
  let displayWeight: number | null | undefined = pet?.weight;
  if (typeof displayWeight === "string") {
    const parsed = Number.parseFloat(displayWeight);
    displayWeight = isNaN(parsed) ? null : parsed;
  }
                  ...
                  <div className="mt-1 text-base font-semibold">
                    {formatNumber(
                      typeof displayWeight === "number"
                        ? Math.round((displayWeight + Number.EPSILON) * 100) / 100
                        : undefined,
                      "kg",
                    )}
                  </div>
